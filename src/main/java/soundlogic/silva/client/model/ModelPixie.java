...
public class ModelPixie extends ModelBase {
	...
	public void setRotationAngles(Pixie pixie, float pticks) {
		RightWing.rotateAngleY = -(MathHelper.cos((pixie.ticks+pticks) * 1.7F) * (float)Math.PI * 0.5F);
		LeftWing.rotateAngleY = MathHelper.cos((pixie.ticks+pticks) * 1.7F) * (float)Math.PI * 0.5F;
	}
}
