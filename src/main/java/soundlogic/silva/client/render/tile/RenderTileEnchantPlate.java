...
import soundlogic.silva.common.block.BlockEnchantPlate;
...
public class RenderTileEnchantPlate extends TileEntitySpecialRenderer {

	@Override
	public void renderTileEntityAt(TileEntity tileentity, double d0, double d1, double d2, float f) {
		TileEnchantPlate plate = (TileEnchantPlate) tileentity;
		
		if(plate.requiredProgress==-1)
			return;

		float max = plate.requiredProgress / 10F;
		float alphaMod = Math.min(max, plate.progress) / max;
		GL11.glPushMatrix();
		...
		renderIcon(0, 0, BlockEnchantPlate.overlay, 1, 1, 240);
